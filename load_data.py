...
class DataGenerator(object):
    ...
    def sample_batch(self, batch_type, batch_size):
        ...
        for _ in range(batch_size):
            ...
            one_hots = np.eye(self.num_classes)
            
            # Sample K+1 for each class
            samples = get_images(N_classes, one_hots, nb_samples=self.num_samples_per_class+1)
            img_paths = samples[:,1]
            all_samples.append(samples)
            all_labels.append(samples[:,0])
        
        all_samples = torch.tensor(all_samples).float().to(self.device)
